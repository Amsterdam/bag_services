...
class KadastraalObject(es.DocType):
    aanduiding = es.String(analyzer=analyzers.postcode, fields={'raw': es.String(index='not_analyzed'), 'ngram': es.String(analyzer=analyzers.kad_obj_aanduiding)})
...
class KadastraalSubject(es.DocType):
    naam = es.String(analyzer=analyzers.naam, fields={'raw': es.String(index='not_analyzed'), 'ngram': es.String(analyzer=analyzers.kad_obj_aanduiding, search_analyzer=analyzers.kad_obj_aanduiding_search)})
