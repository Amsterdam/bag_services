...
class SensitiveDetailsTestCase(APITestCase):
    ...
    def test_niet_ingelogd_geen_details_in_natuurlijk_persoon_json(self):
        response = self.client.get('/api/brk/subject/{}.json'.format(self.natuurlijk.pk)).data
    ...
    def test_niet_ingelogd_wel_details_in_niet_natuurlijk_persoon_json(self):
        response = self.client.get('/api/brk/subject/{}.json'.format(self.niet_natuurlijk.pk)).data
    ...
    def test_ingelogd_zakelijk_recht_verwijst_naar_hoofd_view(self):
        self.client.login(username='authorized', password='pass')
        response = self.client.get('/api/brk/zakelijk-recht/{}.json'.format(self.natuurlijk_recht.pk)).data

        subj = response['kadastraal_subject']
        self.assertEqual(subj['_links']['self']['href'], 'http://testserver/api/brk/subject/{}/'.format(self.natuurlijk.pk))

    def test_uitgelogd_zakelijk_recht_niet_natuurlijk_verwijst_naar_hoofd_view(self):
        response = self.client.get('/api/brk/zakelijk-recht/{}.json'.format(self.niet_natuurlijk_recht.pk)).data

        subj = response['kadastraal_subject']
        self.assertEqual(subj['_links']['self']['href'], 'http://testserver/api/brk/subject/{}/'.format(self.niet_natuurlijk.pk))

    def test_uitgelogd_zakelijk_recht_natuurlijk_verwijst_naar_subresource(self):
        response = self.client.get('/api/brk/zakelijk-recht/{}/'.format(self.natuurlijk_recht.pk)).data

        subj = response['kadastraal_subject']
        self.assertEqual(subj, 'http://testserver/api/brk/zakelijk-recht/{}/subject/'.format(self.natuurlijk_recht.pk))

    def test_subresource_toon_persoonsgegevens_maar_geen_relaties(self):
        response = self.client.get('/api/brk/zakelijk-recht/{}/subject.json'.format(self.natuurlijk_recht.pk)).data
        self.assertIsNotNone(response['woonadres'])
        self.assertIsNotNone(response['postadres'])
        self.assertNotIn('rechten', response)
